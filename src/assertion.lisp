...
context to this informational messsage, like the value of some variables."
  (labels
	 ...
	 (let ((assertion-description-forms
		 (append
		  (when docstring
		    (list `(format assertion-description "~A" ,docstring)))
		  (when report
		    (list `(funcall ,report assertion-description)))))
	       (assertion-body-forms
		 (if (>= 1 (length body))
		     (first body)
		     `(progn ,@body))))
	   ...
	   `(prog1
		(defun ,name ,lambda-list
		  ...
		  (if ,assertion-body-forms
		      t
		      (values
		       nil
		       (with-output-to-string (assertion-description)
			 ,@assertion-description-forms))))
	      (set-assertion-properties ',name ',lambda-list))))
